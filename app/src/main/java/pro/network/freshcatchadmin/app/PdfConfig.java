...
import pro.network.freshcatchadmin.order.Order;
...
public class PdfConfig {
    ...
    public static void addContent(Document document, Order myorderBean, Context context) throws Exception {
        ...
        table20.addCell(createTextCell("Comments :" + (myorderBean.getComments() == null ? "Good" : myorderBean.getComments()), catNormalFont));
        ...
        table21.addCell(createTextCell("Mode :" + myorderBean.getPayment(), catNormalFont));
        ...
        for (int i = 0; i < myorderBean.getProductBeans().size(); i++) {
            Product productListBean = myorderBean.getProductBeans().get(i);
       ...
       // table4.addCell(TableTextRight("Coupon :", catNormalFont));
        ...
        table4.addCell(TableTextRightpaddind(myorderBean.getPrice(), nameFont));
    ...
    public static PdfPCell createTable(PdfPTable pTable, int padding, BaseColor baseColor, boolean isBorder) throws DocumentException, IOException {
        ...
        cell.setBorder(Rectangle.NO_BORDER);
        if (isBorder) {
            cell.setCellEvent((PdfPCellEvent) new DottedCell(PdfPCell.BOX));
        }
